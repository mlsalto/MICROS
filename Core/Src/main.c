...
/* Includes ------------------------------------------------------------------*/
#include "main.h"
#include "stm32f4xx_hal.h" // librería HAL, mirar bien si es esta
#include "string.h"
...
void ponerPieza(uint8_t pieza[4][4], int8_t row, int8_t col);
void eliminarPieza(uint8_t pieza[4][4], int8_t row, int8_t col);
void dibujarMatriz(void);
void dropPiece(uint8_t matrix[16][8], uint8_t piece[4][4]);
...
void dropPiece(uint8_t matrix[16][8], uint8_t piece[4][4]) {
	 eliminarPieza(piece, pieceRow, 2);
	 pieceRow++;
	 ponerPieza(piece, pieceRow, 2);
	 dibujarMatriz();
}
/* USER CODE BEGIN PFP */
